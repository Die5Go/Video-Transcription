...
import os
import io
import shutil  # Importado para ajudar a limpar as pastas
...
if st.session_state.modelo_ia is None:
    try:
        st.session_state.modelo_ia = configurar_ia()
...
def processar_audio(audio_path, metadados_base):
    ...
    with st.spinner("1/2 - Transcrevendo o áudio (isso pode levar alguns minutos)..."):
        try:
            # A função de transcrição agora usa a pasta de output padronizada
            falas = transcrever_audio(audio_path, saida_base=PASTA_OUTPUT)
